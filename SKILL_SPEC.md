...
`soia-open-edu-course-skills` and `soia-private-skills` intentionally use the same template
...
- `hard` / `optional` list SOIA-managed skill names only (published from
  soia-open-edu-course-skills or soia-private-skills).
...
~/.config/soia-skills/soia-open-edu-course-skills/soia-pkm/soia-pkm-transform/config.yml
```
...
`~/.config/soia-skills/soia-open-edu-course-skills/soia-pkm/soia-pkm-transform/notebooklm`.
...
format and is intentionally absent from `soia-open-edu-course-skills`.
