# AGENTS.md - soia-open-edu-course-skills
...
`soia-open-edu-course-skills` publishes reusable `soia-edu-*` skills for the edu domain. Every committed skill must be safe for users who do not share the maintainer's machine, accounts, private data, or internal workspace.
## Routing Boundary
...
- Put user-specific behavior behind CLI args, env vars, or skill-specific
  ...
  `~/.config/soia-skills/soia-open-edu-course-skills/<skill-type>/<skill-name>/config.yml`.
...
```bash
npx skills add soia-team/soia-open-edu-course-skills -l --full-depth
npx skills add soia-team/soia-open-edu-course-skills -g --all
```
...
```bash
claude plugin update soia-edu-course@soia   # 交付走插件市场，勿装全局
```

Forbidden outside local testing:

```bash
npx skills add /absolute/local/path/to/soia-open-edu-course-skills -g -a '*' -s <skill-name> -y
```
...
```bash
cd <your-local-checkout>/soia-open-edu-course-skills
...
3. **Users update** with `claude plugin update soia-edu-course@soia` or
...
claude plugin update soia-edu-course@soia
```
